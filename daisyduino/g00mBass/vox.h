...
namespace synthux
{
  ...
  public:
    ...
    void Init(float sample_rate)
    {
      ...
      _osc2.SetWaveform(Oscillator::WAVE_SIN);
    ...
    void SetOsc1Shape(const float value)
    {
      if (value < .5f)
        ...
        _osc1.SetWaveform(Oscillator::WAVE_POLYBLEP_SQUARE);
    }
    ...
    float Process()
    {
      ...
      if (_env.IsRunning())
      {
        ...
        _osc2.SetFreq(_osc2_freq_mult * _base_freq);

        out += ((_osc1.Process() * osc1_amp) * (1.f - _osc2_amount)) + ((_osc2.Process() * osc2_amp * 2)) * _osc2_amount;
      }
